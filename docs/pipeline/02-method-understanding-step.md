...
- Modular Detail：候选方法筛选、profile 结构、失败回收
...
MethodUnderstandingStep 的职责是：从 `symbols.jsonl` 中选择候选方法并生成 `MethodProfile`，用于后续 Question/Answer 与 Auto Design Questions 的上下文增强。

### 触发条件

- `method_understanding.enabled=true` 且
- Auto QA 开启（未设置 `--skip-question-answer`）**或**
- `question_answer.build_embeddings_in_user_mode=true`
